...
for p in range(len(n_observations)):
    for n_rel in range(len(n_relevants)):
        for i in range(max_models):
            ...
            delta0 = beta.rvs(d, d, size=XDim)
        
            # Measure the execution time of the following code
            start_time = time.time()
            mu, S, invc, pik, Z, lower_bound, Cs, itr = run(X_shuffled, K1, alpha0, beta0, v0, a0, m0, W0, d, delta0, C, 
                                                           threshold, max_itr, T0 = 1. , annealing = "fixed", max_annealed_itr = 10, Ctrick = True)
            end_time = time.time()
